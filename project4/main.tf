...
resource "aws_launch_template" "blue-green" {
  name_prefix   = "blue-green"
  image_id       = data.aws_ami.blue.id  # Replace with the correct AMI
  instance_type  = var.instance_type
  key_name        = var.key_name

  user_data = filebase64("user-data-blue.sh")  # Replace with the appropriate user data script

  tag_specifications {
    resource_type = "instance"
    tags = {
      Name = "blue-green-deployment-template"
    }
  }
...
resource "aws_autoscaling_group" "blue-green" {
  launch_template {
    id      = aws_launch_template.blue-green.id
    version = "$Latest"
  }
  ...
  vpc_zone_identifier = [aws_subnet.pb1.id, aws_subnet.pb2.id]
...
resource "aws_autoscaling_policy" "scale_out" {
  ...
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.blue-green.name
}
